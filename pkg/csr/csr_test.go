...
func TestAddressesEqual(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			nodeName:        "localhost",
			instanceAddress: "invalid",
			output:          false,
			expectedErr:     false,
		},
		{
			name:            "invalid instance IP does not resolve to a DNS",
			nodeName:        "localhost",
			instanceAddress: "127.0.0.2",
			output:          false,
			expectedErr:     true,
		},
	}
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			out, err := mapNodeToInstance(test.instanceAddress, test.nodeName)
			if test.expectedErr {
				assert.Error(t, err)
				assert.False(t, out)
				return
