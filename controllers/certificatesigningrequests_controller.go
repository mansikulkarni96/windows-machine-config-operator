...
func (r *certificateSigningRequestsReconciler) reconcileCSR(request *certificates.CertificateSigningRequest) error {
	certificateSigningRequest, err := csr.NewApprover(r.client, request, r.log, r.recorder, r.watchNamespace)
	if err != nil {
		return errors.Wrapf(err, "could not create WMCO CSR Approver")
	}

	if err := certificateSigningRequest.Approve(r.k8sclientset); err != nil {
